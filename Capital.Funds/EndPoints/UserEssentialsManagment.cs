...
namespace Capital.Funds.EndPoints
{
    public static class UserEssentialsManagment
    {
        ...
        [Authorize(Policy = "UserOnly")]
        public async static Task<IResult> getPaymentsHistory(IUserEssentials _manage, [FromQuery] string tenantId, [FromQuery] int page,[FromQuery] int pageSize)
        {
            ResponseDto responseDto = new() { IsSuccess = false, StatusCode = 400, Message = "", Results = { } };

            PaginatedResult<TenantPayments> result = await _manage.getPaymentsHistoryAsync(tenantId,page, pageSize);
            if (result==null)
            {
                responseDto.StatusCode = 203;
                responseDto.Message = "Data not found";
        ...
        [Authorize(Policy = "UserOnly")]
        public async static Task<IResult> getAllComplaints(IUserEssentials _manage, [FromQuery] string tenantId)
        {
            ResponseDto responseDto = new() { IsSuccess = false, StatusCode = 400, Message = "", Results = { } };

            IEnumerable<TenantComplaints> result = await _manage.getComplaintsAsync(tenantId);
            if (result==null)
            {
                responseDto.StatusCode = 203;
                responseDto.Message = "Data not found";
